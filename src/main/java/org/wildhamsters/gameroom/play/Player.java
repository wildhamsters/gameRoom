...
import org.wildhamsters.gameroom.fleet.ShotResult;

/**
 * Contains all data of Player such as his id, name, board, fleet.
 * ShotVerifier to validate shots.
 * Its methods interact with fleet and board to update it after shot and
 * indicate when player lost the match.
 *
...
class Player {
    ...
    /**
     * Takes index of the board cell, checks if it is in range and gets the
     * FieldState that will be set
     * after shot. If the shot is not the range of board then IllegalShotException
     * is being thrown.
     * <p>
     ...
     * Position is passed to fleet which updates its internals.
     * If ship is sinking then Map of fields to be set as missed shots around the
     * ship is returned.
